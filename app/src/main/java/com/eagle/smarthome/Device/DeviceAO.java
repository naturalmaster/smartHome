...
public class DeviceAO
{
    ...
    public void WriteToStream(OutputStream bw)
    {
        ...
        StreamReadWrite.WriteString(bw, ""+aovalue);//bw.Write((byte)dotype);         //6、DO类型
        StreamReadWrite.WriteString(bw, picture); //bw.Write(pictureoff);           //8、图像文件
        StreamReadWrite.WriteInt(bw, (int)dotplace);//bw.Write((byte)dotype);         //6、DO类型
